...
public:
    ...
    void startup(size_t usersMax, unsigned short port);
    void cleanup();
    void worker();
    ...
    void onDatagramReceive(const sockaddr_in& inetAddr, unsigned char *data, size_t dataSize);
    void send(connectServerUser_t *user, const eMUCore::packet_t &packet);
    void disconnect(connectServerUser_t *user) {
        m_iocpEngine.detach(user);
    }
