...
eMUCORE_DECLSPEC bool eMUCore::isIpAddress(const std::string &hostname) {
...
eMUCORE_DECLSPEC std::string eMUCore::convertToIpAddress(const std::string &hostname) {
    ...
    if(CreateIoCompletionPort(reinterpret_cast<HANDLE>(context->socket()),
        ...
        if(WSARecv(context->socket(),
            ...
            if(wsaLastError != WSA_IO_PENDING) {
                m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::attach()] " << *context
...
void iocpEngine_t::detach(socketContext_t *context) const {
    if(PostQueuedCompletionStatus(m_ioCompletionPort,
        ...
        m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::detach()] " << *context
    ...
    if(!sendBuffer.m_dataLocked) {
        if(dataSize <= c_ioDataMaxSize) {
            ...
            if(WSASend(context->socket(),
                ...
                if(wsaLastError != WSA_IO_PENDING) {
                    m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::write()] " << *context
...
void iocpEngine_t::dequeueReceive(socketContext_t *context) const {
    if(context->recvBuffer().m_dataSize > 0) {
        ...
        if(WSARecv(context->socket(),
            ...
            if(wsaLastError != WSA_IO_PENDING) {
                m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::dequeueReceive()] " << *context
                                                      << " WSARecv() failed with error#" << wsaLastError << ".";
                m_logger->out();
                this->detach(context);
            }
...
void iocpEngine_t::dequeueSend(socketContext_t *context) const {
    ...
    if(sendBuffer.m_secondDataSize > 0) {
        ...
        if(WSASend(context->socket(),
            ...
            if(wsaLastError != WSA_IO_PENDING) {
                m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::dequeueSend()] " << *context
                                                      << " WSASend() failed with error#" << wsaLastError << ".";
                m_logger->out();
                this->detach(context);
            } else {
...
void iocpEngine_t::dequeueError(socketContext_t *context, int lastError) const {
    if(lastError != ERROR_NETNAME_DELETED
        ...
        m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::worker()] GetQueuedCompletionStatus() failed with error# " << lastError
                                              << " but dequeued completion packet.";
        m_logger->out();
    }
    ...
    while(true) {
        ...
        if(completionStatus) {
            ...
            if(buffer->m_type == ioBuffer_e::_recv) {
                ...
                instance->m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::worker()] " << *context
            ...
            if(buffer != NULL) {
                ...
                instance->m_logger->in(loggerMessage_e::_error) << "[iocpEngine_t::worker()] GetQueuedCompletionStatus() failed with error# " << lastError
...
tcpServer_t::tcpServer_t(const callback_t &onAllocate):
    m_logger(NULL),
    m_iocpEngine(NULL),
    m_socket(INVALID_SOCKET),
...
void tcpServer_t::startup(unsigned short listenPort) {
...
DWORD tcpServer_t::worker(tcpServer_t *instance) {
    while(true) {
        ...
        if(incomingSocket != INVALID_SOCKET) {
            instance->m_logger->in(loggerMessage_e::_info) << "Connection attempt from address: " << inet_ntoa(incomingInetAddr.sin_addr) << ".";
            instance->m_logger->out();
            ...
            instance->m_logger->in(loggerMessage_e::_error) << "[tcpServer_t::worker()] WSAAccept failed with error#" << WSAGetLastError << ".";
            instance->m_logger->out();
        }
    }
...
udpSocket_t::udpSocket_t(const callback_t &onReceive):
    m_logger(NULL),
    m_onReceive(onReceive),
    m_socket(INVALID_SOCKET),
    m_port(0) {}

void udpSocket_t::startup(unsigned short port) {
    ...
    if(ret > 0) {
        if(FD_ISSET(m_socket, &fdSet)) {
            ...
            if(ret > 0)	{
                ...
                m_logger->in(loggerMessage_e::_error) << "[udpSocket_t::worker()] recvfrom() failed with error#" << WSAGetLastError() << ".";
                m_logger->out();
            }
        }
    } else if(ret == SOCKET_ERROR) {
        m_logger->in(loggerMessage_e::_error) << "[udpSocket_t::worker()] select() failed with error#" << WSAGetLastError() << ".";
        m_logger->out();
    }
}
...
void udpSocket_t::send(std::string hostname,
    ...
    if(isIpAddress(hostname)) {
        ...
        if(ipAddress != "") {
            ...
            m_logger->in(loggerMessage_e::_error)  << "[udpSocket_t::send()] Invalid hostname: " << hostname << " specified.";
            m_logger->out();
            return;
        }
    }
    ...
    do {
        ...
        if(ret != SOCKET_ERROR) {
            ...
            m_logger->in(loggerMessage_e::_error) << "[udpSocket_t::send()] sendto() failed with error#" << WSAGetLastError() << ".";
            m_logger->out();
            break;
        }
