...
TEST(readBufferTest, construct) {
    ...
    EXPECT_EQ(eMUCore::network::maxPayloadSize_, rbuf.payload_.size());
}

TEST(readBufferTest, clear) {
    eMUCore::network::readBuffer_t rbuf;
    rbuf.payload_[0] = 0x00;
    rbuf.payload_[eMUCore::network::maxPayloadSize_ - 1] = 0x01;

    rbuf.clear();

    ASSERT_EQ(eMUCore::network::maxPayloadSize_, rbuf.payload_.size());
    EXPECT_EQ(0x00, rbuf.payload_[0]);
    EXPECT_EQ(0x00, rbuf.payload_[eMUCore::network::maxPayloadSize_ - 1]);
}
...
protected:
    ...
    void initializePatternBuf() {
        ...
        patternBuf_.payloadSize_ = eMUCore::network::maxPayloadSize_;
    }
...
TEST_F(writeBufferTest, construct) {
    ...
    EXPECT_EQ(eMUCore::network::maxPayloadSize_, wbuf.payload_.size());
    EXPECT_EQ(0, wbuf.payloadSize_);
    EXPECT_EQ(eMUCore::network::maxPayloadSize_, wbuf.secPayload_.size());
...
TEST_F(writeBufferTest, clear) {
    ...
    wbuf.payload_[eMUCore::network::maxPayloadSize_ - 1] = 0x01;
    wbuf.payloadSize_ = 2;

    wbuf.secPayload_[0] = 0x02;
    wbuf.secPayload_[eMUCore::network::maxPayloadSize_ - 1] = 0x03;
    ...
    ASSERT_EQ(eMUCore::network::maxPayloadSize_, wbuf.payload_.size());
    EXPECT_EQ(0x00, wbuf.payload_[0]);
    EXPECT_EQ(0x00, wbuf.payload_[eMUCore::network::maxPayloadSize_ - 1]);
    EXPECT_EQ(0, wbuf.payloadSize_);

    ASSERT_EQ(eMUCore::network::maxPayloadSize_, wbuf.secPayload_.size());
    EXPECT_EQ(0x00, wbuf.secPayload_[0]);
    EXPECT_EQ(0x00, wbuf.secPayload_[eMUCore::network::maxPayloadSize_ - 1]);
    EXPECT_EQ(0, wbuf.secPayloadSize_);
...
TEST_F(writeBufferTest, insert_PendingState) {
