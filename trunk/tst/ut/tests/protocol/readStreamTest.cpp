...
TEST_F(ReadStreamTest, readNext)
{
    ...
    uint16_t &value2 = reinterpret_cast<uint16_t&>(samplePayloads_.fullFilledPayload_[6]);
    ASSERT_EQ(value2, readStream_.readNext<uint16_t>());

    uint8_t value3 = samplePayloads_.fullFilledPayload_[8];
...
{
    size_t string1Length = 10;
    std::string string1(reinterpret_cast<const char*>(&samplePayloads_.fullFilledPayload_[2]), string1Length);
    ASSERT_EQ(string1, readStream_.readNextString(string1Length));

    size_t string2Length = 20;
    std::string string2(reinterpret_cast<const char*>(&samplePayloads_.fullFilledPayload_[12]), string2Length);
    ASSERT_EQ(string2, readStream_.readNextString(string2Length));
}

TEST_F(ReadStreamTest, getId)
{
    uint16_t &id = reinterpret_cast<uint16_t&>(samplePayloads_.fullFilledPayload_[0]);
